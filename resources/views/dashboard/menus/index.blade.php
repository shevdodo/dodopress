<x-layouts.dashboard title="Menu Management">
    ...
    <script>
        ...
        let menuItemBaseUrl = "{{ route('superuser.menus.items.update', ['item' => '__ID__']) }}".replace('__ID__', '');
        if (window.location.protocol === 'https:' && menuItemBaseUrl.startsWith('http:')) {
            menuItemBaseUrl = menuItemBaseUrl.replace('http:', 'https:');
        }
